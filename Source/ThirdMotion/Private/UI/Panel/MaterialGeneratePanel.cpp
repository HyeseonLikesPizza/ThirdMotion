...
bool UMaterialGeneratePanel::Initialize()
{
	...
	// PreviewMaterialRenderer = NewObject<UPreviewMaterialRenderer>();

		TArray<AActor*> FoundActors;
	UGameplayStatics::GetAllActorsOfClass(GetWorld(), APreviewImageGenerator::StaticClass(), FoundActors);

	if (FoundActors.Num() > 0)
	{
		// 첫 번째(그리고 유일한) 액터를 멤버 변수에 저장
		PreviewImagen = Cast<APreviewImageGenerator>(FoundActors[0]);
	}
